...
def RunMainLoop():
 global can_paint,mouse_down,brush_type,mouseX,mouseY,current_color,info_text,size_pixels,color_text,mouse_text,pixel_text,layercode,layers,pixels,screen,w1,w2,screen_division,pixels_last,error,altpressed,history_count,SPEED,pixel_animation,saved_file,tag,count_error,altwaspressed
 import os
 while True:
...
#    mouse_text = f"Mouse Position: {pygame.mouse.get_pos()}"
  final_text = color_text + " | " + f"Brush Type: {brush_type}" + " | "+f"{info_text if info_text!='' else '...'} | "+f"Layer: {layercode} of {layers}"+f" | "+f"FPS: {int(clock.get_fps())}"+" | "+f"{len(pixels)} pixels in scene"+" | "+"Press F1 for controls" 
   ...
   if event.type == pygame.MOUSEWHEEL:
     ...
     if event.y == -1:
        if pygame.key.get_mods() & KMOD_CTRL and SPEED > 10:
             ...
             size_pixels -= 2
     info_text=f"Pixel Size: {size_pixels}"
